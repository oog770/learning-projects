(function () {
  const newBookmarkForm = document.querySelector("#bookmark-item-input-form");
  const bookmarkItemList = document.querySelector("#bookmark-list");
  let bookmarkList = [];
  ...
  const addBookmarkItem = () => {
    ...
    let name = document.querySelector("#new-bookmark-name-input").value;
    let url = document.querySelector("#new-bookmark-url-input").value;
    let createAt = Date.now();
    bookmarkList.push({ name: name, url: url, createAt: createAt });
    localStorage.setItem("bookmarkList", JSON.stringify(bookmarkList));
    document.querySelector("#new-bookmark-name-input").value = "";
    document.querySelector("#new-bookmark-url-input").value = "";
  ...
  const deleteBookMarkItem = (id) => {
    ...
    if (isDelete) {
      ...
      document.querySelector(`#bookmark-item-${id}`).remove();
    }
  };
  ...
  document
    .querySelector(".bookmark-item-add-btn")
    .addEventListener("click", newBookmarkToggle);

  document.querySelector("#add-btn").addEventListener("click", addBookmarkItem);
  document
